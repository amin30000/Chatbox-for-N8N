<?php
/*
Plugin Name: Brand Chatbot for n8n
Description: A brandable chat widget that connects to an n8n Webhook. Adds a shortcode [brand_chatbot] and optional auto-injection.
Version: 1.0.0
Author: Your Name
License: MIT
*/
...
class Brand_Chatbot_Plugin {
    const OPTION_KEY = 'brand_chatbot_options';
    ...
    public function __construct() {
        ...
        add_shortcode('brand_chatbot', [$this, 'shortcode_handler']);
    }
    ...
    public function register_settings() {
        ...
        add_settings_section('brand_chatbot_main', 'Chatbot Settings', function () {
        ...
        foreach ($fields as [$key, $label, $type]) {
            ...
            }, 'brand-chatbot', 'brand_chatbot_main');
        }
    }
    ...
    public function render_settings_page() {
        ...
        echo '<div class="wrap">';
        echo '<h1>Brand Chatbot</h1>';
        echo '<form method="post" action="options.php">';
        settings_fields(self::OPTION_KEY);
        do_settings_sections('brand-chatbot');
        submit_button();
        echo '</form>';
        echo '<p><strong>Shortcode:</strong> [brand_chatbot]</p>';
        echo '<p><strong>Note:</strong> For headers/extra context, provide valid JSON objects.</p>';
        echo '</div>';
    }

    public function enqueue_frontend_assets() {
        $handle = 'brand-chatbot-widget';
        $src = plugins_url('assets/js/brand-chatbot.js', __FILE__);
    ...
    private function output_init_script($overrides = []) {
        ...
        echo "\n<script>\n(function(){\n  try {\n    var cfg = $config_json;\n    cfg.transformRequest = function(text, ctx){ return { message: text, sessionId: ctx.sessionId, metadata: ctx.metadata }; };\n    cfg.transformResponse = function(data){\n      if (data && typeof data === 'object') {\n        if (Object.prototype.hasOwnProperty.call(data, '$response_field')) return data['$response_field'];\n        if (Object.prototype.hasOwnProperty.call(data, 'message')) return data['message'];\n        if (Object.prototype.hasOwnProperty.call(data, 'text')) return data['text'];\n      }\n      if (typeof data === 'string') return data;\n      return 'No response';\n    };\n    if (window.BrandChatbot && typeof window.BrandChatbot.init === 'function') {\n      window.BrandChatbot.init(cfg);\n    }\n  } catch(e) { console && console.error && console.error(e); }\n})();\n</script>\n";
    }
    ...
    public function shortcode_handler($atts = []) {
        ...
        foreach ($atts as $k => $v) {
            if ($v !== '') {
                if (in_array($k, ['open_by_default','dark_mode','allow_html'], true)) {
                    $overrides[$k] = filter_var($v, FILTER_VALIDATE_BOOLEAN);
                } elseif (in_array($k, ['z_index','max_messages'], true)) {
